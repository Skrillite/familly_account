    ...
    async with session.begin():
        (account_id,) = (
            await session.execute(
                insert(DBMembers)
                .from_select(
                    [DBMembers.user_id, DBMembers.account_id],
                    select(data.changing_user_id, DBMembers.account_id).where(
                        DBMembers.user_id == data.requesting_user_id
                    ),
                )
...
async def delete_member(session: AsyncSession, deleting_user_id) -> int:
    async with session.begin():
        (account_id,) = (
            await session.execute(
                select(DBMembers.account_id).where(
                    DBMembers.user_id == deleting_user_id
                )
            )
        ).fetchone()
